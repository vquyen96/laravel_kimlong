        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">{{ $web_info->ad_acc }}</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
        ...
        <div class="col-12">
          ...
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">{{ $web_info->ad_acc_list }}</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              ...
              <table id="example1" class="table table-bordered table-hover">
                <thead>
                <tr>
                  <th class="hideResponsive768">{{ $web_info->ad_acc_image }}</th>
                  <th>{{ $web_info->ad_acc_username }}</th>
                  <th class="hideResponsive768">{{ $web_info->ad_acc_fullname }}</th>
                  <th class="hideResponsive768">{{ $web_info->ad_acc_email }}</th>
                  <th>{{ $web_info->ad_acc_option }}</th>
                </tr>
                </thead>
                <tbody>
                	...
                	<tr>
	                  ...
	                  <td>
                        ...
                        <a href="{{ asset('admin/account/edit/'.$item->id) }}" class="btn btn-primary">{{ $web_info->ad_acc_edit }}</a>
                        <a href="{{ asset('admin/account/delete/'.$item->id) }}" onclick="return confirm('Bạn chắc chắn muốn sáp nhập')" class="btn btn-danger">{{ $web_info->ad_acc_delete }}</a>
                      @endif
