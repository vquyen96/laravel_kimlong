...
@section('title', $web_info->ad_profile)
@section('main')
...
<div class="content-wrapper">
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>{{ $web_info->ad_profile_detail}}</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ asset('admin') }}">{{ $web_info->ad_home}}</a></li>
            <li class="breadcrumb-item active">{{ $web_info->ad_profile}}</li>
          </ol>
        </div>
      </div>
    </div><!-- /.container-fluid -->
        ...
        <div class="col-md-4">
          ...
          <div class="card card-danger card-outline">
            <div class="card-body box-profile">
              ...
              <a href="{{ asset('admin/profile/change_pass') }}" class="btn btn-danger btn-block"><b>{{ $web_info->ad_profile_changepass}}</b></a>
            </div>
            <!-- /.card-body -->
          </div>
        </div>
        <div class="col-md-8">
          <div class="card card-danger">
            <div class="card-header">
              <h3 class="card-title">{{ $web_info->ad_profile_detail}}</h3>
            </div>
            <!-- /.card-header -->
            <!-- form start -->
            <form role="form" method="post" enctype="multipart/form-data">
              <div class="card-body">
                <div class="form-group">
                    <label for="exampleInputEmail1">{{ $web_info->ad_profile_username}}</label>
                    <input type="text" class="form-control" placeholder="Username" name="username" value="{{$item->username}}" disabled>
                </div>
                <div class="form-group">
                    <label for="exampleInputEmail1">{{ $web_info->ad_profile_fullname}}</label>
                    <input type="fullname" class="form-control" placeholder="Fullname" name="fullname" value="{{$item->fullname}}">
                </div>
                <div class="form-group">
                    <label for="exampleInputEmail1">{{ $web_info->ad_profile_email}}</label>
                    <input type="email" class="form-control" placeholder="Enter email" name="email" value="{{$item->email}}"> 
                </div>
                <div class="form-group">
                    <label for="exampleInputPassword1">{{ $web_info->ad_profile_phone}}</label>
                    <input type="text" class="form-control" data-inputmask='"mask": "(999) 999-9999"' data-mask name="phone" value="{{$item->phone}}" >
                </div>
                <div class="form-group">
                    <label for="exampleInputFile">{{ $web_info->ad_profile_image}}</label>
                    
                    <div>
                      <input id="img" type="file" name="img" class="cssInput" onchange="changeImg(this)" style="display: none!important;">
                      <img style="cursor: pointer;" id="avatar" class="cssInput thumbnail imageForm" src="{{ isset($item->img) && file_exists(storage_path('app/avatar/'.$item->img)) && $item->img ? asset('local/storage/app/avatar/'.$item->img) : 'images/default-image.png' }}">
                    </div>
              ...
              <div class="card-footer">
                <input type="submit" class="btn btn-primary" value="{{ $web_info->ad_profile_edit }}">
                {{csrf_field()}}
              </div>
